...
state_sigma = [0.03]' ;                   % Generate state noise sequence for simulation 
wk =  state_sigma * randn(n_ud, N_samples) ;
meas_sigma  = [0.03]';                      % Generate state noise sequence for simulation 
vk = meas_sigma * randn(1, N_samples) ;        

% Use only the first row of C_mat if you want only the first output
C_mat_single = C_mat(3,:);        % CHANGE: Using only first row of output matrix
...
for k = 2 : N_samples 
    ...
    if ( mod_type == 0 )     % Process simulation using discrete linear perturbation model
        ...
        Yk_abs(k) = Ys(3) + yk(k) ;               % CHANGE: Using only first element of Ys
        ...
        Yk_abs(k) = yk(k) + Ys(3) ;                    % CHANGE: Using only first element of Ys
