...
state_sigma = [0.1]' ;                   % Generate state noise sequence for simulation 
...
C_mat_single = C_mat(1,:);        % CHANGE: Using only first row of output matrix
...
for k = 2 : N_samples 
    ...
    if ( mod_type == 0 )     % Process simulation using discrete linear perturbation model
        ...
        Yk_abs(k) = Ys(1) + yk(k) ;               % CHANGE: Using only first element of Ys
        ...
        Yk_abs(k) = yk(k) + Ys(1) ;                    % CHANGE: Using only first element of Ys
